...
- **Auto-Generate Missing Documentation (`--create-missing`)**: Added the `--create-missing` flag to `sync_cmd` inside `main.py`. This option automatically creates empty `.md` documentation pages for any undocumented Python modules, bootstraps them with a clean title header, and invokes the AI engine to write a comprehensive technical reference from scratch.
- **Bootstrapping Prompt Guidance**: Upgraded the AI prompt templates in `ai_engine.py` to identify empty or header-only documents and trigger a complete reference generation instead of a non-destructive patch.

### Changed

- **Version Upgrades**: Incremented project and package versions from `0.3.1` to `0.4.0` in package configurations.
